...
gen_SCBM_function2 :-
    ...
    write('Sinit_back(&&'),write(P),write('_'),write(A),write(',arglist,th);'),nl,
    gen_pack_pointer([],0),
    write('Spush_next(&&success,th);'),nl,
...
gen_SCBM_function4 :-
    ...
    write('if(rp[th]==1) np[th] = 1;'),nl,
    write('else Spush_next(&&success,th);'),nl,
    write('goto *next;'),nl,
    write('}else{'),nl,
...
% B=before nondet [A,M,N] for generate fail continuation
...
gen_nondet_body1((X,Y),A,M,N,H,P,V,D,B) :-
    ...
    gen_nondet_body1(Y,A,M,N1,H,P,V,D,[P,A,M,N]).
